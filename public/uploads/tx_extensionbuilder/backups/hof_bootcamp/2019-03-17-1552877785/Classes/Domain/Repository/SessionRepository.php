...
class SessionRepository extends \TYPO3\CMS\Extbase\Persistence\Repository
{
     ...
     */
    public function getParticipantSessions(\HofBootCamp\HofBootcamp\Domain\Model\Participant $participant)
    {
        $query = $this->createQuery();
        $query->matching($query->contains('participants', $participant));
        return $query->execute();
    }
}
